...
import { getPaquete, updatePaquete, Elemento, Paquete } from "../../services/paquetesService";
...
export default function EditarPaqueteForm() {
  ...
  useEffect(() => {
    ...
    if (!id) {
      setError("ID de paquete no válido");
      setLoading(false);
      return;
    }

    getPaquete(id)
      .then((paquete: Paquete) => {
        form.reset({
          nombre: paquete.nombre,
          descripcion: paquete.descripcion || "",
          activo: paquete.activo,
          elementos: paquete.elementos,
          totalPaquete: paquete.totalPaquete,
      ...
      .catch((err: any) => {
        logger.error("Error loading paquete:", err);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      // Incluir descuentos solo si hay valores válidos
      if (data.descuentoTipo) {
        payload.descuentoTipo = data.descuentoTipo;
      }
      if (data.descuentoValor !== undefined && data.descuentoValor !== 0) {
        payload.descuentoValor = data.descuentoValor;
      }

      await updatePaquete(id, payload);
      navigate("/paquetes");
    } catch (err: any) {
      logger.error("Error updating paquete:", err);
      handleError(err);
      if (!errorMessage) {
        setError("Error al actualizar el paquete. Intenta de nuevo.");
      }
    } finally {
      setSaving(false);
    }
  ...
  if (loading) {
    return (
      <FormPageShell>
        <div className="rounded-2xl bg-white p-8 text-center shadow-sm">Cargando paquete...</div>
      </FormPageShell>
    );
  }

  if (error === "Paquete no encontrado") {
    return (
      <FormPageShell>
        <div className="rounded-2xl bg-white p-6 shadow-lg">
          <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4">
            ...
            <div>
              <h3 className="font-semibold text-red-900">Paquete no encontrado</h3>
              <p className="mt-1 text-sm text-red-700">El paquete que intentas editar no existe o ha sido eliminado.</p>
            </div>
          </div>
          <div className="mt-4">
            <Button onClick={handleCancel} className="w-full">Volver a Paquetes</Button>
          </div>
        </div>
      </FormPageShell>
    );
  ...
  return (
    <>
      <FormPageShell>
        {(error || errorMessage) && error !== "Paquete no encontrado" && (
          <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} onKeyDown={preventEnterFormSubmit} className="space-y-5">
          <FormSection
            ...
            description="Actualiza la información base y los descuentos sin alterar la lógica actual de validación y guardado."
          >
            <FieldGrid>
          ...
          <FormSection
            title="Resumen"
            description="Verifica el total calculado antes de guardar los cambios del paquete."
          >
